...
function createConfigFile(apiHost, apiProxy, apiSslVerify, apiUserAgent) {
  apiHost = sanitize(apiHost);
  apiProxy = sanitize(apiProxy);
  apiSslVerify = sanitize(apiSslVerify);
  apiUserAgent = sanitize(apiUserAgent);
  if (apiHost) {
...
`;
    ...
    // Write config file
    fs.writeFileSync(configPath, configContent, { encoding: 'utf8', mode: 0o600 });
