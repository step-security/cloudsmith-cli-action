...
function validateApiHost(host) {
  if (!/^[a-zA-Z0-9.-]+$/.test(host)) {
    throw new Error(`Invalid api-host value: "${host}". Must be a plain hostname.`);
  }
  return host;
}
...
async function authenticate(
  ...
  const baseUrl = `https://${apiHost ? validateApiHost(apiHost) : DEFAULT_API_HOST}`;
  let idToken;
  let token;
