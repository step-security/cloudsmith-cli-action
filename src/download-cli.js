...
async function installCliViaPip() {
  try {
    ...
    // Use Cloudsmith as the primary index to avoid dependency confusion: pip
    // resolves `cloudsmith-cli` from Cloudsmith's index first. PyPI is added
    // as a fallback so transitive dependencies (click, etc.) still resolve.
    await exec.exec('pip', [
      'install',
